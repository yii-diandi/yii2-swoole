...
\diandi\swoole\server\Server::run($config,function ($nodeConfig){
    $server = \diandi\swoole\server\Server::autoCreate($nodeConfig);
    $starter = new \diandi\swoole\bootstrap\YiiWeb($server);
